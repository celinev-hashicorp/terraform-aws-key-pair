...
output "key_pair_id" {
  ...
  value       = try(aws_key_pair.public_key[0].key_pair_id, "")
}

output "key_pair_arn" {
  description = "The key pair ARN"
  value       = try(aws_key_pair.public_key[0].arn, "")
}

output "key_pair_name" {
  description = "The key pair name"
  value       = try(aws_key_pair.public_key[0].key_name, "")
}

output "key_pair_fingerprint" {
  description = "The MD5 public key fingerprint as specified in section 4 of RFC 4716"
  value       = try(aws_key_pair.public_key[0].fingerprint, "")
}
...
output "private_key_id" {
  ...
  value       = try(tls_private_key.private_key[0].id, "")
}

output "private_key_openssh" {
  description = "Private key data in OpenSSH PEM (RFC 4716) format"
  value       = try(trimspace(tls_private_key.private_key[0].private_key_openssh), "")
  sensitive   = true
}

output "private_key_pem" {
  description = "Private key data in PEM (RFC 1421) format"
  value       = try(trimspace(tls_private_key.private_key[0].private_key_pem), "")
  sensitive   = true
}
...
output "public_key_fingerprint_md5" {
  ...
  value       = try(tls_private_key.private_key[0].public_key_fingerprint_md5, "")
}
...
output "public_key_fingerprint_sha256" {
  ...
  value       = try(tls_private_key.private_key[0].public_key_fingerprint_sha256, "")
}
...
output "public_key_openssh" {
  ...
  value       = try(trimspace(tls_private_key.private_key[0].public_key_openssh), "")
}

output "public_key_pem" {
  description = "Public key data in PEM (RFC 1421) format"
  value       = try(trimspace(tls_private_key.private_key[0].public_key_pem), "")
}
